...
def parse_args():
    ...
    ap.add_argument("--robustness_ratios", type=str, default="0,0.05,0.10,0.20",
                    help="Contamination ratios for source-side robustness analysis.")
    ap.add_argument("--robustness_repeats", type=int, default=5,
                    help="Random repeats per contamination ratio.")
    ap.add_argument("--robustness_seed", type=int, default=2026)
...
def write_notes(path: str, args, src_sel: List[CompareRecord], tgt_sel: List[CompareRecord],
                ...
                static_metrics: Dict[str, object], robust_summary: pd.DataFrame, contrib_summary: pd.DataFrame):
    lines = []
    ...
    lines.append("")
    lines.append("## Static Prototype Similarity Checks")
    cos = static_metrics.get("cosine_per_class", [])
    l2g = static_metrics.get("relative_l2_gap_per_class", [])
    lines.append(f"- cosine(mean_proto, nmf_proto) per class: `{[round(float(v), 6) for v in cos]}`")
    lines.append(f"- relative L2 gap per class: `{[round(float(v), 6) for v in l2g]}`")
    lines.append(f"- target prediction agreement (nearest-prototype): `{float(static_metrics.get('target_prediction_agreement', float('nan'))):.6f}`")
    lines.append(f"- correlation(mean response map, nmf response map): `{float(static_metrics.get('response_map_correlation', float('nan'))):.6f}`")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Source-side Robustness (new focus)")
    if robust_summary is not None and len(robust_summary) > 0:
        b0 = robust_summary.iloc[0]
        blast = robust_summary.iloc[-1]
        lines.append(
            "- Prototype drift @ max contamination: "
            f"mean={float(blast['drift_mean_avg_mean']):.6f}, nmf={float(blast['drift_nmf_avg_mean']):.6f}"
        )
        lines.append(
            "- Target nearest-prototype accuracy drop (max contamination - clean): "
            f"mean={float(blast['acc_mean_mean'] - b0['acc_mean_mean']):.6f}, "
            f"nmf={float(blast['acc_nmf_mean'] - b0['acc_nmf_mean']):.6f}"
        )
        lines.append(
            "- Target margin drop (max contamination - clean): "
            f"mean={float(blast['margin_mean_mean'] - b0['margin_mean_mean']):.6f}, "
            f"nmf={float(blast['margin_nmf_mean'] - b0['margin_nmf_mean']):.6f}"
        )
    lines.append("")
    lines.append("## NMF Sample Contribution (new focus)")
    if contrib_summary is not None and len(contrib_summary) > 0:
        for _, r in contrib_summary.iterrows():
            lines.append(
                f"- class {int(r['class'])}: N={int(r['num_samples'])}, "
                f"uniform={float(r['uniform_weight']):.6f}, max_w={float(r['max_weight']):.6f}, "
                f"ESS/N={float(r['ess_ratio']):.6f}, gini={float(r['gini']):.6f}"
            )
    lines.append("")
    if bool(static_metrics.get("static_similarity_is_close", False)):
        lines.append("## Conclusion")
        lines.append("- Current static prototype visualization alone cannot support a strong claim that `NMF prototype` is clearly better than `mean prototype`.")
        lines.append("- The stronger evidence should come from robustness under source contamination and from sample-contribution interpretability.")
        lines.append("")
...
def main():
    ...
    print("[Step] extracting source/target features for robustness + contribution analyses...")
    src_feats, src_labels, _ = extract_domain_features(model, src_loader, has_label=True, device=device)
    tgt_feats, tgt_labels, _ = extract_domain_features(model, tgt_loader, has_label=tgt_has_label, device=device)
    src_feats_by_cls = group_source_features_by_class(src_feats, src_labels, int(args.num_classes))

    proto_mean_cls = class_proto_vectors(proto_wo, int(args.num_classes))
    proto_nmf_cls = class_proto_vectors(proto_w, int(args.num_classes))
    static_metrics = compute_static_similarity_metrics(
        proto_mean_cls=proto_mean_cls,
        proto_nmf_cls=proto_nmf_cls,
        tgt_feats=tgt_feats,
        tgt_labels=tgt_labels,
        tgt_records=tgt_records,
    )
    print("[Static] cosine(mean_proto, nmf_proto) per class:", static_metrics["cosine_per_class"])
    print("[Static] relative L2 gap per class:", static_metrics["relative_l2_gap_per_class"])
    print("[Static] target prediction agreement (mean-vs-nmf nearest-prototype):", static_metrics["target_prediction_agreement"])
    print("[Static] response map correlation (mean-vs-nmf):", static_metrics["response_map_correlation"])

    print("[Step] running source-side robustness analysis...")
    robust = run_source_robustness_analysis(args, src_feats_by_cls=src_feats_by_cls, tgt_feats=tgt_feats, tgt_labels=tgt_labels)
    robust_per_run: pd.DataFrame = robust["per_run"]
    robust_summary: pd.DataFrame = robust["summary"]

    print("[Step] running NMF sample contribution analysis...")
    contrib = run_nmf_sample_contribution_analysis(
        src_feats_by_cls=src_feats_by_cls,
        num_classes=int(args.num_classes),
        nmf_max_iter=int(args.nmf_max_iter),
    )
    contrib_long: pd.DataFrame = contrib["weights_long"]
    contrib_summary: pd.DataFrame = contrib["summary"]
    ...
    out_robust_png = os.path.join(args.outdir, "source_robustness_analysis.png")
    out_robust_pdf = os.path.join(args.outdir, "source_robustness_analysis.pdf")
    out_robust_csv = os.path.join(args.outdir, "source_robustness_per_run.csv")
    out_robust_summary_csv = os.path.join(args.outdir, "source_robustness_summary.csv")
    out_contrib_png = os.path.join(args.outdir, "nmf_sample_contribution.png")
    out_contrib_pdf = os.path.join(args.outdir, "nmf_sample_contribution.pdf")
    out_contrib_csv = os.path.join(args.outdir, "nmf_sample_contribution_weights.csv")
    out_contrib_summary_csv = os.path.join(args.outdir, "nmf_sample_contribution_summary.csv")
    ...
    draw_source_robustness_figure(out_robust_png, out_robust_pdf, robust, num_classes=int(args.num_classes), has_target_label=bool(tgt_has_label))
    draw_nmf_contribution_figure(out_contrib_png, out_contrib_pdf, contrib, num_classes=int(args.num_classes))
    robust_per_run.to_csv(out_robust_csv, index=False)
    robust_summary.to_csv(out_robust_summary_csv, index=False)
    contrib_long.to_csv(out_contrib_csv, index=False)
    contrib_summary.to_csv(out_contrib_summary_csv, index=False)
    write_selected_cases(out_csv, src_records, tgt_records, src_sel, tgt_sel, used_src=src_case.case_id, used_tgt=tgt_case.case_id)
    write_notes(out_notes, args, src_sel, tgt_sel, src_case, tgt_case, src_metrics, static_metrics, robust_summary, contrib_summary)
    ...
    meta = {
        ...
        "static_similarity_metrics": static_metrics,
        "robustness_ratios": parse_float_list(args.robustness_ratios),
        "robustness_repeats": int(args.robustness_repeats),
        "robustness_summary_rows": robust_summary.to_dict(orient="records"),
        "contribution_summary_rows": contrib_summary.to_dict(orient="records"),
    }
    ...
    print(f"[Done] {out_robust_png}")
    print(f"[Done] {out_robust_pdf}")
    print(f"[Done] {out_robust_csv}")
    print(f"[Done] {out_robust_summary_csv}")
    print(f"[Done] {out_contrib_png}")
    print(f"[Done] {out_contrib_pdf}")
    print(f"[Done] {out_contrib_csv}")
    print(f"[Done] {out_contrib_summary_csv}")
